...
import {
  ...
  CircularProgress, Chip, IconButton, Tooltip, Tabs, Tab
} from '@mui/material'
...
export default function Search() {
  ...
  useEffect(() => {
    ...
    async function loadDoctors() {
      ...
      try {
        // fetchAll: true ile tüm sayfaları çek (arama için gerekli)
        const list = await getAllDoctors(token, { signal: controller.signal, fetchAll: true })
        if (!mounted) return
        // list artık direkt array dönüyor (fetchAll: true ile)
  ...
  useEffect(() => {
    ...
    async function loadUsers() {
      ...
      try {
        // fetchAll: true ile tüm sayfaları çek (arama için gerekli)
        const list = await getAllPublicUsers(token, { signal: controller.signal, fetchAll: true })
        if (!mounted) return
        // list artık direkt array dönüyor (fetchAll: true ile)
        const data = Array.isArray(list) ? list : (list?.content || [])
        const seen = new Set()
  ...
  return (
    <Box sx={{ py: { xs: 1, md: 0 }, px: { xs: 0, sm: 0 } }}>
      {/* Başlık ve Sekmeler */}
      <Stack spacing={{ xs: 1.5, md: 2.5 }} sx={{ mb: { xs: 2, md: 4 }, px: { xs: 1.5, md: 3 }, pt: { xs: 1.5, md: 3 }, borderBottom: '1px solid', borderColor: 'divider' }}>
