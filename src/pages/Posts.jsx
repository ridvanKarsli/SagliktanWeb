...
export default function Posts() {
  ...
  async function loadMorePosts() {
    ...
    try {
      ...
      if (category && category.trim()) {
        response = await getChatsWithFilter(token, category.trim(), { page: nextPage })
      } else {
        response = await getAllPosts(token, { page: nextPage })
      }
      ...
      const mapped = topLevelPosts.map(post => {
  ...
  async function loadChats(useFilter = false, resetPagination = true) {
    const isLoadingInitial = !useFilter || resetPagination
    if (isLoadingInitial) {
      setLoading(true)
      if (resetPagination) {
        setCurrentPage(0)
    ...
    try {
      ...
      if (category && category.trim()) {
        try {
          response = await getChatsWithFilter(token, category.trim(), { page: 0 })
        } catch (filterError) {
          ...
          if (allPosts.length > 0) {
            ...
            response = await getAllPosts(token, { page: 0 })
          }
        }
      } else {
      ...
      setTotalElements(response?.totalElements || fetchedPosts.length)
      setHasMore(response?.hasNext || false)
      ...
      try {
        ...
        console.debug('count:', data.length, 'category:', category || 'none', 'page:', response?.currentPage, 'totalPages:', response?.totalPages)
  ...
  useEffect(() => {
    ...
    loadChats(false, true) // İlk yükleme, pagination sıfırla
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      loadChats(true, true) // Filtreleme, pagination sıfırla
    }, 300) // 300ms debounce
            ...
            <Box sx={{ textAlign: 'center', py: 2 }}>
              <Typography variant="caption" sx={{ color: 'text.secondary' }}>
                {totalElements > sorted.length 
                  ? `${sorted.length} / ${totalElements} gönderi`
                  : `${sorted.length} gönderi`
                }
              </Typography>
            </Box>
          )}
